...
*/
char **split_token(char **spltstr, char *s, unsigned int i, char del[])
{
	char *token;
...
/**
 * spit_command - takes the command string to substring
 *
*/
char **split_command(char *buff_str, char del[])
{
	...
	spltstr = split_token(spltstr, tmp_split, m, del);
	tmp_split = '\0';
	free(tmp_split);
	return (spltstr);
}
